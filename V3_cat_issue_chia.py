...
async def get_signed_tx_not_use(fingerprint, ph, amt, fee):
    try:
...
def cli(
    ...
    CHIVES_CAT_XCH_ISSUE_DOING = r.hgetall("CHIVES_CAT_XCH_ISSUE_DOING")
    COUNTER = 0
    for CHIVES_CAT_XCH_ISSUE_DOING_KEY,CHIVES_CAT_XCH_ISSUE_DOING_STATUS in CHIVES_CAT_XCH_ISSUE_DOING.items():
        if CHIVES_CAT_XCH_ISSUE_DOING_STATUS != "DONE":
            CHIVES_CAT_XCH_ISSUE_APPLY_ITEM = r.hget("CHIVES_CAT_XCH_ISSUE_APPLY",CHIVES_CAT_XCH_ISSUE_DOING_KEY)
            if CHIVES_CAT_XCH_ISSUE_APPLY_ITEM is not None:
                CHIVES_CAT_XCH_ISSUE_APPLY_JSON = json.loads(CHIVES_CAT_XCH_ISSUE_APPLY_ITEM)
                if CHIVES_CAT_XCH_ISSUE_APPLY_JSON['ServiceFeeSendStatus'] == "DONE" and CHIVES_CAT_XCH_ISSUE_APPLY_JSON['SelectCoinMakeStatus'] == "DONE" and CHIVES_CAT_XCH_ISSUE_APPLY_JSON['IssueAssetTokenStatus'] !="DONE":
                    print(CHIVES_CAT_XCH_ISSUE_APPLY_JSON)
                    send_to = str(CHIVES_CAT_XCH_ISSUE_APPLY_JSON['ISSUE_TO_ADDRESS'])
                    amount = uint64(CHIVES_CAT_XCH_ISSUE_APPLY_JSON['ISSUE_TO_AMOUNT'])
                    amount = amount*1000 #此处指的是发行CAT的数量,所以要乘以1000
                    DEPOSIT_COINS = CHIVES_CAT_XCH_ISSUE_APPLY_JSON['DEPOSIT_COINS'][0]
                    #break;
                    #传递参数,固定写法,不需要更改
                    tail = "/home/ubuntu/CAT-admin-tool/reference_tails/genesis_by_coin_id.clsp.hex"
                    ...
                    #    get_signed_tx(fingerprint, cat_ph, amount, fee,DEPOSIT_COINS) 
                    #)
                    #print('signed_tx------------------------------------------------------')
                    #print(signed_tx.spend_bundle)
                    wt = WalletTool(DEFAULT_CONSTANTS)
                    signed_tx = asyncio.get_event_loop().run_until_complete(
                        wt.get_signed_tx(str(cat_ph), amount, fee,DEPOSIT_COINS)
                    )
                    #print('signed_tx------------------------------------------------------')
                    if signed_tx=="NO COIN SELECTED" :
                        #当选择COIN操作过后五分钟,还是得不到COIN,就标记为NO COIN SELECTED,然后重新选择COIN
                        LeftTime = int(time.time()) - int(str(CHIVES_CAT_XCH_ISSUE_APPLY_JSON['SelectCoinMakeTime']))
                        print(f"发行金额: {CHIVES_CAT_XCH_ISSUE_APPLY_JSON['ISSUE_TO_AMOUNT']} 间隔时间: {LeftTime} 秒 发行地址: {CHIVES_CAT_XCH_ISSUE_APPLY_JSON['ISSUE_TO_ADDRESS']}")
                        if(LeftTime>300):
                            CHIVES_CAT_XCH_ISSUE_APPLY_JSON['SelectCoinMakeStatus'] = "NO COIN SELECTED"
                            CHIVES_CAT_XCH_ISSUE_APPLY_JSON['SelectCoinMakeTime'] = ""
                            CHIVES_CAT_XCH_ISSUE_APPLY_JSON['SelectCoinMakeHASH_KEY'] = ""
                            r.hset("CHIVES_CAT_XCH_ISSUE_APPLY",CHIVES_CAT_XCH_ISSUE_DOING_KEY, json.dumps(CHIVES_CAT_XCH_ISSUE_APPLY_JSON) )
                        continue;
                    #print('additions------------------------------------------------------')
                    ...
                    #print("未提交到网络: 调试模式: AssetID:")
                    #print(str(curried_tail.get_tree_hash()))
                    #return
                    ...
                    if push_transaction_result['success'] == True and push_transaction_result['status'] == "SUCCESS":
                        print("已经提交到网络.每一个CAT的生成需要3-5分钟的时间间隔,不然会有其中一个CAT会因为选择了同样的COIN,导致创建失败")
                        print(push_transaction_result)                         
                        CHIVES_CAT_XCH_ISSUE_APPLY_JSON['IssueAssetTokenStatus'] = "DONE"
                        CHIVES_CAT_XCH_ISSUE_APPLY_JSON['IssueAssetTokenTime'] = int(time.time())
                        CHIVES_CAT_XCH_ISSUE_APPLY_JSON['IssueAssetTokenHASH_KEY'] = ""
                        CHIVES_CAT_XCH_ISSUE_APPLY_JSON['ASSET_ID'] = push_transaction_result['AssetID']
                        r.hset("CHIVES_CAT_XCH_ISSUE_APPLY",CHIVES_CAT_XCH_ISSUE_DOING_KEY, json.dumps(CHIVES_CAT_XCH_ISSUE_APPLY_JSON) )
...
class WalletTool:
    ...
    async def  get_signed_tx(self,SendToPuzzleHash,SendToAmount,fee,DEPOSIT_COINS):           
        ...
        '''
        #实际过程中,只能选择一个COIN,并且这个COIN会提前生成
        AllPuzzleHashArrayText = "'"+DEPOSIT_COINS['puzzlehash']+"'"
        #缓存私钥
        self.puzzle_hash_to_private_key[DEPOSIT_COINS['puzzlehash']] = DEPOSIT_COINS
        print(self.puzzle_hash_to_private_key[DEPOSIT_COINS['puzzlehash']]['private_key'])
        ...
        log = logging.Logger
        db_connection = await aiosqlite.connect("/home/ubuntu/.chia/standalone_wallet/db/blockchain_v1_mainnet.sqlite")
        ...
        for row in rows:
            ...
            #print(SendToAmount)
            #print(row) 
            #构建CAT时,只能选择一个COIN,并用是金额相等的情况.不能通过选择多个COIN来累计金额的形式实现.
    ...
    def generate_unsigned_transaction(
        ...
        for n, coin in enumerate(coins):
            ...
            #puzzle = puzzle_for_pk(bytes(pubkey))
            #print(puzzle_hash)
            #print(self.puzzle_hash_to_private_key[str(puzzle_hash)])
            #print('----------------------')
            public_key = hexstr_to_bytes(self.puzzle_hash_to_private_key[str(puzzle_hash)]['public_key'])
    ...
    def sign_transaction(self, coin_solutions: List[CoinSpend]) -> SpendBundle:
        ...
        for coin_solution in coin_solutions:  # type: ignore # noqa
            ...
            secret_key = self.puzzle_hash_to_private_key[str(coin_solution.coin.puzzle_hash)]['private_key']
