...
def cli(
    ...
    CHVIES_CAT_ISSUE_DOING = r.hgetall("CHVIES_CAT_ISSUE_DOING")
    COUNTER = 0
    for CHVIES_CAT_ISSUE_DOING_KEY,CHVIES_CAT_ISSUE_DOING_STATUS in CHVIES_CAT_ISSUE_DOING.items():
        if CHVIES_CAT_ISSUE_DOING_STATUS != "DONE":
            CHVIES_CAT_ISSUE_APPLY_ITEM = r.hget("CHVIES_CAT_ISSUE_APPLY",CHVIES_CAT_ISSUE_DOING_KEY)
            if CHVIES_CAT_ISSUE_APPLY_ITEM is not None:
                CHVIES_CAT_ISSUE_APPLY_JSON = json.loads(CHVIES_CAT_ISSUE_APPLY_ITEM)
                if CHVIES_CAT_ISSUE_APPLY_JSON['ServiceFeeSendStatus'] == "DONE" and CHVIES_CAT_ISSUE_APPLY_JSON['SelectCoinMakeStatus'] == "DONE" and CHVIES_CAT_ISSUE_APPLY_JSON['IssueAssetTokenStatus'] !="DONE":
                    print(CHVIES_CAT_ISSUE_APPLY_JSON)
                    send_to = str(CHVIES_CAT_ISSUE_APPLY_JSON['ISSUE_TO_ADDRESS'])
                    amount = uint64(CHVIES_CAT_ISSUE_APPLY_JSON['ISSUE_TO_AMOUNT'])
                    amount = amount*1000 #此处指的是发行CAT的数量,所以要乘以1000
                    DEPOSIT_COINS = CHVIES_CAT_ISSUE_APPLY_JSON['DEPOSIT_COINS'][0]
                    #break;
                    #传递参数,固定写法,不需要更改
                    ...
                    if signed_tx=="NO COIN SELECTED" :
                        ...
                        LeftTime = int(time.time()) - int(str(CHVIES_CAT_ISSUE_APPLY_JSON['SelectCoinMakeTime']))
                        print(f"发行金额: {CHVIES_CAT_ISSUE_APPLY_JSON['ISSUE_TO_AMOUNT']} 间隔时间: {LeftTime} 秒 发行地址: {CHVIES_CAT_ISSUE_APPLY_JSON['ISSUE_TO_ADDRESS']}")
                        if(LeftTime>300):
                            CHVIES_CAT_ISSUE_APPLY_JSON['SelectCoinMakeStatus'] = "NO COIN SELECTED"
                            CHVIES_CAT_ISSUE_APPLY_JSON['SelectCoinMakeTime'] = ""
                            CHVIES_CAT_ISSUE_APPLY_JSON['SelectCoinMakeHASH_KEY'] = ""
                            r.hset("CHVIES_CAT_ISSUE_APPLY",CHVIES_CAT_ISSUE_DOING_KEY, json.dumps(CHVIES_CAT_ISSUE_APPLY_JSON) )
                        continue;
                    #print('additions------------------------------------------------------')
                    ...
                    if push_transaction_result['success'] == True and push_transaction_result['status'] == "SUCCESS":
                        ...
                        CHVIES_CAT_ISSUE_APPLY_JSON['IssueAssetTokenStatus'] = "DONE"
                        CHVIES_CAT_ISSUE_APPLY_JSON['IssueAssetTokenTime'] = int(time.time())
                        CHVIES_CAT_ISSUE_APPLY_JSON['IssueAssetTokenHASH_KEY'] = ""
                        CHVIES_CAT_ISSUE_APPLY_JSON['ASSET_ID'] = push_transaction_result['AssetID']
                        r.hset("CHVIES_CAT_ISSUE_APPLY",CHVIES_CAT_ISSUE_DOING_KEY, json.dumps(CHVIES_CAT_ISSUE_APPLY_JSON) )
